...
This role is used for updating and recovering the rook jenkins in the rook ci vpc.

Those are the functions in this role are

- rook-jenkins-update For updating rook jenkins version to the version defined in the "jenkins_master_image" varible

- rook-os-update For updating rook jenkins OS packages

- rook-recovery For recoverying the Prod-jenkins instance from the image defined in the "image" varible in a case that the instance was deleted or crashed
...
**Pre-requisites:** Before running ``rook.yml`` make sure your IP address has ssh access to the VPC. this is configured in the [AWS dashboard](https://console.aws.amazon.com/ec2/v2/home?region=us-east-1#Home:), under the "rook-jenkins-group" security group inbound rules.
...
- AWS dashboard access
  Access details to the AWS dashboard can be found in here http://wiki.ceph.redhat.com/dokuwiki/doku.php?id=rook_aws_account
...
The rook jenkins image this is the backup image used when creating the recovery the rook jenkins::
...
- rook-jenkins-update
    Update the rook jenkins app to the version defined in the "jenkins_master_image varible".

- rook-os-update
    Update the rook jenkins OS packages.

- rook-recovery
    Recover the rook jenkins instance from the image defined in "image" varible.
